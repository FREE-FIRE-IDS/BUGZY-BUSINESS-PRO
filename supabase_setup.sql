...
BEGIN
  ...
  -- 1. Extract company_id and validate access
    ...
    FOR v_item IN SELECT jsonb_array_elements(req_payload) LOOP
      ...
      IF v_company_id IS NULL AND req_table NOT IN ('profiles', 'licenses', 'payment_requests') THEN
         RAISE EXCEPTION 'Identification (id/company_id) is required for table %', req_table;
      END IF;
      ...
      IF v_item IS NULL OR jsonb_typeof(v_item) = 'null' THEN v_item := '{}'::jsonb; END IF;
  ...
  ELSE
    v_item := COALESCE(req_payload, '{}'::jsonb);
    IF jsonb_typeof(v_item) = 'null' THEN v_item := '{}'::jsonb; END IF;
