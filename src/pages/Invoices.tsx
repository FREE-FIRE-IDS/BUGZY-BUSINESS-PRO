...
export default function Invoices() {
  ...
  const handleAddItem = () => {
    setSelectedItems([...selectedItems, { item_id: '', quantity: 1, price: 0 }]);
  };
  ...
  const exportInvoicePDF = (invoice: any) => {
    ...
    doc.text(`Date: ${formatDate(invoice.date)}`, 140, 41);
    if (invoice.due_date) doc.text(`Due Date: ${formatDate(invoice.due_date)}`, 140, 47);
    ...
    doc.setTextColor(100, 116, 139);
    doc.text('Tax (17%):', 130, finalY + 7);
    doc.setTextColor(30, 41, 59);
    doc.text(formatCurrency(invoice.tax, settings.currency), 196, finalY + 7, { align: 'right' });
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-black">Invoices</h2>
          <p className="text-slate-500">Manage and generate professional invoices</p>
        </div>
        <button 
          onClick={() => {
      ...
      <div className="relative max-w-md">
        ...
        <input 
          ...
          className="w-full pl-12 pr-4 py-3 bg-white border border-slate-200 rounded-2xl focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-black"
        />
      </div>
        ...
        {filteredInvoices.map((invoice) => {
          ...
          return (
            ...
            >
              ...
              <h3 className="font-bold text-lg mb-1 text-black">{invoice.invoice_number}</h3>
              <p className="text-sm text-slate-500 mb-4">{party?.name || 'Unknown Party'}</p>
              <div className="flex justify-between items-end">
                <div>
                  <p className="text-xs text-slate-400 uppercase tracking-wider font-bold">Total Amount</p>
                  <p className="text-xl font-bold text-black dark:text-white">{formatCurrency(invoice.total, settings.currency)}</p>
                </div>
                <span className={cn(
      ...
      <AnimatePresence>
        {isAddModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-4xl bg-white dark:bg-slate-900 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <form className="p-8 space-y-8 overflow-y-auto" onSubmit={(e) => {
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div>
                    ...
                    <input name="date" type="date" required defaultValue={new Date().toISOString().split('T')[0]} className="w-full p-3 rounded-xl border border-slate-200 dark:border-slate-700 dark:bg-slate-800 outline-none focus:ring-2 focus:ring-indigo-500" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-500 mb-1">Due Date</label>
                    <input name="due_date" type="date" className="w-full p-3 rounded-xl border border-slate-200 dark:border-slate-700 dark:bg-slate-800 outline-none focus:ring-2 focus:ring-indigo-500" />
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    {selectedItems.map((si, index) => (
                      <div key={index} className="flex gap-3 items-end">
                        ...
                        <button type="button" onClick={() => removeItem(index)} className="p-3 text-rose-600 hover:bg-rose-50 rounded-xl transition-all">
                          <Trash2 size={20} />
                        </button>
                      </div>
                    ))}
                ...
                <div className="flex justify-end pt-8">
                  <div className="w-64 space-y-2">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-bold">{formatCurrency(selectedItems.reduce((sum, si) => sum + (si.quantity * si.price), 0) * 0.17, settings.currency)}</span>
                    </div>
                    <div className="flex justify-between text-xl pt-2 border-t border-slate-100 dark:border-slate-800">
                      <span className="font-bold">Total</span>
                      <span className="font-bold text-indigo-600">{formatCurrency(selectedItems.reduce((sum, si) => sum + (si.quantity * si.price), 0) * 1.17, settings.currency)}</span>
                    </div>
                  </div>
                </div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-4xl bg-white dark:bg-slate-900 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <form className="p-8 space-y-8 overflow-y-auto" onSubmit={(e) => {
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  <div>
                    <label className="block text-sm font-medium text-slate-500 mb-1">Party</label>
                    <select name="party_id" defaultValue={editingInvoice.party_id} required className="w-full p-3 rounded-xl border border-slate-200 dark:border-slate-700 dark:bg-slate-800 outline-none focus:ring-2 focus:ring-indigo-500">
                      {parties.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-500 mb-1">Invoice Date</label>
                    <input name="date" type="date" required defaultValue={editingInvoice.date.split('T')[0]} className="w-full p-3 rounded-xl border border-slate-200 dark:border-slate-700 dark:bg-slate-800 outline-none focus:ring-2 focus:ring-indigo-500" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-500 mb-1">Due Date</label>
                    <input name="due_date" type="date" defaultValue={editingInvoice.due_date?.split('T')[0]} className="w-full p-3 rounded-xl border border-slate-200 dark:border-slate-700 dark:bg-slate-800 outline-none focus:ring-2 focus:ring-indigo-500" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-500 mb-1">Status</label>
                    <select name="status" defaultValue={editingInvoice.status} className="w-full p-3 rounded-xl border border-slate-200 dark:border-slate-700 dark:bg-slate-800 outline-none focus:ring-2 focus:ring-indigo-500">
